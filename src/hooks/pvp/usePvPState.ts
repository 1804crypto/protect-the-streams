...
import { Streamer, applyNatureToStats } from '@/data/streamers';
import { useCollectionStore, getNature } from '../useCollectionStore';
import { useVisualEffects } from '../useVisualEffects';
import { PvPPlayerState, PvPActionPayload } from '@/types/pvp';
...
export const usePvPState = (
    ...
    const [lastAction, setLastAction] = useState<PvPActionPayload | null>(null);
    ...
    useEffect(() => {
        if (!isSpectatorMode && player) {
            setIntegrity(player.hp / player.maxHp);
        }
    }, [player, setIntegrity, isSpectatorMode]);
