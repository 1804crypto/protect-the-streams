...
export const MediaUplink: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-20 left-6 z-50 flex flex-col items-start gap-4 pointer-events-none">
            ...
            <motion.div
                initial={{ opacity: 0, x: -20, scale: 0.95, pointerEvents: 'none' }}
                animate={{
                    opacity: isOpen ? 1 : 0,
                    x: isOpen ? 0 : -20,
                    scale: isOpen ? 1 : 0.95,
                ...
                className="glass-card w-80 p-4 border border-neon-purple/30 bg-black/80 backdrop-blur-xl shadow-[0_0_30px_rgba(168,85,247,0.2)] overflow-hidden"
            >
                {/* Header */}
